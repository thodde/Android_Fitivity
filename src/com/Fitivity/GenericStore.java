...
public class GenericStore {
	...
	private static String PREF_FILE_NAME = "Fitivity_appPrefs";
	...
	private static void clearCache(Context context, String caller) {
		((ApplicationFitivity) context.getApplicationContext())
				.getObjectCache().removeAllObjects();
		((ApplicationFitivity) context.getApplicationContext()).getImageCache()
				.removeAllObjects();
	...
	private static boolean saveObjectInCache(String objKey,
			Serializable objData, Context context) {
		if (!getCustomBoolean(SETTINGS_ISCACHEHAVEDATA, context))
			setCustomData(TYPE_SHAREDPREF, SETTINGS_ISCACHEHAVEDATA, true,
					context);

		ObjectCache objCache = ((ApplicationFitivity) context
				.getApplicationContext()).getObjectCache();
		return objCache.saveObject(objKey, objData);
	}

	private static boolean saveObject(String objKey, Serializable dataObj,
			Context context) {
		Editor editor = context.getSharedPreferences(PREF_FILE_NAME,
				Context.MODE_PRIVATE).edit();
	...
	private static Object getObject(String objKey, Context context) {
		...
		Object dataObj = ObjectSerializer.deserialize(savedSession.getString(
				objKey, null));
	...
	public static boolean isCustomKeyExist(int type, String customKey,
			Context context) {
		if (TYPE_MEMDISKCACHE == type)
	...
	private static boolean isCustomKeyExistInCache(String objKey,
			Context context) {
		ObjectCache objCache = ((ApplicationFitivity) context
				.getApplicationContext()).getObjectCache();
	...
	private static boolean isCustomKeyExistInLocal(String customKey,
			Context context) {
		SharedPreferences savedSession = context.getSharedPreferences(
				PREF_FILE_NAME, Context.MODE_PRIVATE);
	...
	private static boolean setCustomDataInLocal(String key, Object value,
			Context context) {
	...
	public static boolean getCustomBoolean(String key, boolean defValue,
			Context context) {
		SharedPreferences savedSession = context.getSharedPreferences(
				PREF_FILE_NAME, Context.MODE_PRIVATE);
	...
	public static String getCustomString(String key, String defValue,
			Context context) {
		SharedPreferences savedSession = context.getSharedPreferences(
				PREF_FILE_NAME, Context.MODE_PRIVATE);
	...
	public static float getCustomFloat(String key, float defValue,
			Context context) {
		SharedPreferences savedSession = context.getSharedPreferences(
				PREF_FILE_NAME, Context.MODE_PRIVATE);
	...
	public static void checkAddCustomPrefix(String prefix, Context context) {
		...
		if (!isCustomKeyExist(GenericStore.TYPE_MEMDISKCACHE,
				CUSTOMPREFIXES_INCACHE, context)) {

			prefixesInCache.add(prefix);
			if (LOGGING.DEBUG)
				Log.d(TAG, CUSTOMPREFIXES_INCACHE
						+ " doesNOT exist, create NEW|" + prefix + "|"
						+ prefixesInCache.toString());

			saveObject(GenericStore.TYPE_MEMDISKCACHE, CUSTOMPREFIXES_INCACHE,
					prefixesInCache, context);
		} else {
			prefixesInCache = (CustomPrefixesInCache) getObject(
					GenericStore.TYPE_MEMDISKCACHE, CUSTOMPREFIXES_INCACHE,
					context);
			if (!prefixesInCache.contains(prefix)) {
				...
				if (LOGGING.DEBUG)
					Log.d(TAG, CUSTOMPREFIXES_INCACHE
							+ " do exist, add and save|" + prefix + "|"
							+ prefixesInCache.toString());

				saveObject(GenericStore.TYPE_MEMDISKCACHE,
						CUSTOMPREFIXES_INCACHE, prefixesInCache, context);
			}
		}
	}
		...
		if (isCustomKeyExist(GenericStore.TYPE_MEMDISKCACHE,
			...
			if (prefixesInCache.contains(prefix)) {
				...
				if (prefixesInCache.size() == 0)
					removeObject(GenericStore.TYPE_MEMDISKCACHE,
							CUSTOMPREFIXES_INCACHE, context);
				else
					saveObject(GenericStore.TYPE_MEMDISKCACHE,
							CUSTOMPREFIXES_INCACHE, prefixesInCache, context);
	...
	public static void removeAllCustomPrefixes(Context context) {
		if (isCustomKeyExist(GenericStore.TYPE_MEMDISKCACHE,
				CUSTOMPREFIXES_INCACHE, context)) {
			CustomPrefixesInCache prefixesInCache = (CustomPrefixesInCache) getObject(
					GenericStore.TYPE_MEMDISKCACHE, CUSTOMPREFIXES_INCACHE,
					context);
			...
			if (LOGGING.DEBUG)
				Log.d(TAG,
						CUSTOMPREFIXES_INCACHE + "| will clear all data for "
								+ prefixesInCache.toString());
			for (String customPrefix : customPrefixes)
				removeObject(GenericStore.TYPE_MEMDISKCACHE, customPrefix,
						context);
			removeObject(GenericStore.TYPE_MEMDISKCACHE,
					CUSTOMPREFIXES_INCACHE, context);
		} else {
