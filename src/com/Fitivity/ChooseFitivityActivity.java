...
public class ChooseFitivityActivity extends ExpandableListActivity {
		...
		try {
			...
			expListAdapter = new SimpleExpandableListAdapter(this,
					...
					R.layout.child_row, // Layout for sub-level entries(second
										// level).
					new String[] { "subcategory" }, // Keys in childData maps to
													// display.
	...
	/*
	 * Used for pulling activities out of the database (right now they are
	 * hard-coded) NOT TESTED AT ALL YET public void attemptQuery() { ParseQuery
	 * query = new ParseQuery("Activity"); //Fetch all of the activities that
	 * are in the database if (query != null) {
	 * query.orderByAscending("category"); query.addAscendingOrder("name");
	 * query.setLimit(200); query.whereExists("category");
	 * 
	 * query.findInBackground(new FindCallback() { public void
	 * done(List<ParseObject> results, ParseException e) {
	 * ArrayList<ParseObject> categoryArray = new ArrayList<ParseObject>(); if(e
	 * == null) { String lastCategory = ""; boolean firstTry = true;
	 * 
	 * for(int i = 0; i <= results.size(); i++) {
	 * if(lastCategory.equalsIgnoreCase(results.get(i).toString())) {
	 * categoryArray.add(results.get(i)); } else { if (firstTry) { firstTry =
	 * false; } else { categories.addAll(categoryArray); }
	 * 
	 * lastCategory = results.get(i).toString(); categoryArray = null; } }
	 * categories.addAll(categoryArray); categoryArray = null;
	 * 
	 * ArrayList<ParseObject> resultsToShow = new ArrayList<ParseObject>(); for
	 * (int i = 0; i < categories.size(); i++) { resultsToShow.add(new
	...
	public boolean onChildClick(ExpandableListView parent, View v,
		...
		Object o = (Object) expListAdapter.getChild(groupPosition,
				childPosition);
