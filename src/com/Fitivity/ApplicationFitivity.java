...
public class ApplicationFitivity extends Application {
	...
	public void onLowMemory() {
		...
		// clearing image cache should be enough, feel free to also clear object
		// cache
		clearImageCacheFu();
	}
	...
	public void clearImageCacheFu() {
		...
		if (LOGGING.DEBUG)
			Log.d(TAG, "cleared " + itemCount + " items from ImageCache" + "|"
	...
	public void clearObjectCacheFu() {
		...
		if (LOGGING.DEBUG)
			Log.d(TAG, "cleared " + itemCount + " items from ObjectCache" + "|"
	...
	public synchronized Context getActiveContext(String className) {
		...
		if (c == null) // If the WeakReference is no longer valid, ensure it is
						// removed.
	...
	public ObjectCache getObjectCache() {
		...
		if (objectCache == null) {
			if (LOGGING.DEBUG)
				Log.d(TAG, "objectCache: init NEW");
			objectCache = new ObjectCache(INITIAL_CAPACITY_objectCache,
					DEFAULT_TTL_MINUTES_objectCache,
					DEFAULT_POOL_SIZE_objectCache);
			objectCache.setCacheDirName(mainCacheDirName_AppCache);// main cache
																	// dir name
			objectCache.enableDiskCache(this, ObjectCache.DISK_CACHE_SDCARD);

			if (!GenericStore.isCustomKeyExist(GenericStore.TYPE_MEMDISKCACHE,
					GenericStore.KEY_OBJECTCACHEFUPATH, this))
				GenericStore.setCustomData(GenericStore.TYPE_SHAREDPREF,
						GenericStore.KEY_OBJECTCACHEFUPATH,
						objectCache.getDiskCacheDirectory(), this);
		}
	...
	public ImageCache getImageCache() {
		...
		if (imageCache == null) {
			if (LOGGING.DEBUG)
				Log.d(TAG, "imageCache: init NEW");
			imageCache = new ImageCache(INITIAL_CAPACITY_ImageCache,
					DEFAULT_TTL_MINUTES_ImageCache,
					DEFAULT_POOL_SIZE_ImageCache);
			imageCache.setCacheDirName(mainCacheDirName_AppCache);// main cache
																	// dir name
			imageCache.enableDiskCache(this, ObjectCache.DISK_CACHE_SDCARD);

			if (!GenericStore.isCustomKeyExist(GenericStore.TYPE_MEMDISKCACHE,
					GenericStore.KEY_IMAGECACHEFUPATH, this))
				GenericStore.setCustomData(GenericStore.TYPE_SHAREDPREF,
						GenericStore.KEY_IMAGECACHEFUPATH,
						imageCache.getDiskCacheDirectory(), this);
		}
