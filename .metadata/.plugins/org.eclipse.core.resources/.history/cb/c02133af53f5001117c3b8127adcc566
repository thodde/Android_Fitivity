...
public class GenericStore {
	...
	private static void clearCache(Context context, String caller) {
		((ApplicationFitivity)context.getApplicationContext()).getObjectCache().removeAllObjects();
		((ApplicationFitivity)context.getApplicationContext()).getImageCache().removeAllObjects();
    ...
    private static boolean saveObjectInCache(String objKey, Serializable objData, Context context) {
    	...
    	ObjectCache objCache = ((ApplicationFitivity)context.getApplicationContext()).getObjectCache();
